...
def find_pressor(vasodf=None, output=None, debug=False, find_high_hrs=24):
    ...
    # Find max for eat row in output
    NE_equiv = find_current.groupby(['patient_id', 'encounter_id', 'score_time', 'pressor_name'])['ne_eq'].max().reset_index()
    NE_equiv = NE_equiv.groupby(['patient_id', 'encounter_id', 'score_time'])['ne_eq'].sum().reset_index()

    # Find iano
    iano = find_current.groupby(['patient_id', 'encounter_id', 'score_time'])['ianotrope'].max().reset_index()
...
def score_qSOFA(lvdf=None, adt=None, calc_interval_mins=None, LOCF_hours=None, debug=False, gcs_cutoff=13, cutoff=2):
...
def score_SOFA(lvdf=None, adt=None, mvdf=None, dxdf=None, vasodf=None, uodf=None,
